...
//  std::tuple<float, float, float> res;
  ...
  auto x = point.x;
  auto y = point.y;
  alpha = (x * (b[1] - c[1]) + (c[0] - b[0]) * y + b[0] * c[1] - c[0] * b[1]) / (
      a[0] * (b[1] - c[1]) + (c[0] - b[0]) * a[1] + b[0] * c[1] - c[0] * b[1]);
  beta = (x * (c[1] - a[1]) + (a[0] - c[0]) * y + c[0] * a[1] - a[0] * c[1]) / (
      b[0] * (c[1] - a[1]) + (a[0] - c[0]) * b[1] + c[0] * a[1] - a[0] * c[1]);
  gamma = (x * (a[1] - b[1]) + (b[0] - a[0]) * y + a[0] * b[1] - b[0] * a[1]) / (
      c[0] * (a[1] - b[1]) + (b[0] - a[0]) * c[1] + a[0] * b[1] - b[0] * a[1]);
